...
void Asteroid_Spawn(Asteroid *a, Vector2 playerPos)
{
    ...
    Vector2 dir = {
        playerPos.x - a->physics.position.x,
...
void Asteroid_Draw(const Asteroid *a)
{
    ...
    switch (a->type.shape)
    {
        case ASTEROID_SQUARE:
            DrawRectangle(
                (int)(a->physics.position.x - a->type.geometry.length / 2.0f),
                (int)(a->physics.position.y - a->type.geometry.height / 2.0f),
                (int)a->type.geometry.length,
                (int)a->type.geometry.height,
                a->type.color);
            break;
        ...
        {
            Vector2 top = {
                a->physics.position.x,
                a->physics.position.y - a->type.geometry.height / 2.0f
            };

            Vector2 left = {
                a->physics.position.x - a->type.geometry.length / 2.0f,
                a->physics.position.y + a->type.geometry.height / 2.0f
            };
